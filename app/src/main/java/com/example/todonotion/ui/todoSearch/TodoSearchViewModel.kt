...
class TodoSearchViewModel @Inject constructor(private val keywordsRepository: KeywordsRepository) : ViewModel() {
    ...
    private val _storeKeyword = MutableLiveData<Keyword?>()
    val storeKeyword: LiveData<Keyword?> = _storeKeyword
    ...
    private fun updateKeyword(keyword: Keyword) {
        val newKeyword = getUpdateKeyword(
            keywordId = keyword.id,
            keywordName = keyword.keyName,
            keywordComplete = !keyword.isCompleted
        )
    ...
    private fun filterKeywords(keywords: List<Keyword>, filteringType: KeywordsFilterType) {
